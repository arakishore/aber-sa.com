...
<?php if (isset($my_drivers) && sizeof($my_drivers)>0) {  ?>                            
                                <table class="table align-items-center table-flush table-borderless">
                                  <thead>
                                   ...
                                   <tbody>
<?php foreach($my_drivers as $key => $value){  ?>                                  
                                    <tr>
                                        <td><?php echo $this->common->getDbValue($value['first_name']); ?> <?php echo $this->common->getDbValue($value['last_name']); ?></td>
                                        ...
                                        <td>
                                        <?php if($value['status_flag']=='Active'){?>
                                        <i class="fa fa-check green" aria-hidden="true"></i></i>
                                        <?php } else { ?>
                                        <i class="fa fa-times red" aria-hidden="true"></i>
                                        <?php } ?>
                                        </td>
                                        <td>
                                        <a href="<?php echo site_url('serviceprovider/edit_driver/'.$this->common->getDbValue($value['user_id']))?>">
                                        <i class="fas fa-pen"></i> Edit</a></td>
                                    </tr>
<?php } ?>
